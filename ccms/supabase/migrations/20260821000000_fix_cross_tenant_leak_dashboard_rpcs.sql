...
CREATE OR REPLACE FUNCTION public.get_age_breakdown(p_org_id uuid, p_branch_id uuid DEFAULT NULL::uuid)
RETURNS TABLE(age_group text, cnt bigint) LANGUAGE sql STABLE AS $$
  SELECT
    ...
    COUNT(*)
  FROM members
  WHERE org_id = p_org_id AND p_org_id = get_my_org_id() AND (p_branch_id IS NULL OR branch_id = p_branch_id)
    AND membership_status = 'active' AND date_of_birth IS NOT NULL
  GROUP BY 1 ORDER BY MIN(date_of_birth) DESC;
$$;

CREATE OR REPLACE FUNCTION public.get_attendance_by_event_type(p_org_id uuid, p_branch_id uuid DEFAULT NULL::uuid, p_start date DEFAULT NULL::date, p_end date DEFAULT NULL::date)
RETURNS TABLE(event_type text, avg_rate numeric, event_count bigint, total_present bigint) LANGUAGE sql STABLE AS $$
  ...
  WHERE e.org_id = p_org_id AND p_org_id = get_my_org_id() AND (p_branch_id IS NULL OR e.branch_id = p_branch_id)
...
CREATE OR REPLACE FUNCTION public.get_branch_comparison(p_org_id uuid, p_start date DEFAULT NULL::date, p_end date DEFAULT NULL::date)
RETURNS TABLE(branch_id uuid, branch_name text, member_count bigint, monthly_giving numeric, attendance_rate numeric) LANGUAGE sql STABLE AS $$
  SELECT b.id, b.name,
  ...
  LEFT JOIN events e ON e.branch_id = b.id AND e.org_id = p_org_id
    ...
    AND (p_end IS NULL OR e.starts_at::date <= p_end)
  LEFT JOIN attendance a ON a.event_id = e.id AND a.org_id = p_org_id
  WHERE b.org_id = p_org_id AND p_org_id = get_my_org_id()
  GROUP BY b.id, b.name ORDER BY COUNT(DISTINCT m.id) DESC;
$$;

CREATE OR REPLACE FUNCTION public.get_category_breakdown(p_org_id uuid, p_branch_id uuid DEFAULT NULL::uuid, p_start date DEFAULT NULL::date, p_end date DEFAULT NULL::date)
RETURNS TABLE(category text, total numeric) LANGUAGE sql STABLE AS $$
  SELECT COALESCE(tc.name, 'Other'), COALESCE(SUM(t.amount), 0)
  FROM transactions t
  LEFT JOIN transaction_categories tc ON tc.id = t.category_id
  WHERE t.org_id = p_org_id AND p_org_id = get_my_org_id() AND (p_branch_id IS NULL OR t.branch_id = p_branch_id)
    AND (p_start IS NULL OR t.transaction_date >= p_start)
    AND (p_end IS NULL OR t.transaction_date <= p_end)
  GROUP BY COALESCE(tc.name, 'Other') ORDER BY SUM(t.amount) DESC;
$$;

CREATE OR REPLACE FUNCTION public.get_gender_breakdown(p_org_id uuid, p_branch_id uuid DEFAULT NULL::uuid)
RETURNS TABLE(gender text, cnt bigint) LANGUAGE sql STABLE AS $$
...
CREATE OR REPLACE FUNCTION public.get_giving_by_branch(p_org_id uuid, p_start date DEFAULT NULL::date, p_end date DEFAULT NULL::date)
RETURNS TABLE(branch_id uuid, branch_name text, total numeric) LANGUAGE sql STABLE AS $$
...
RETURNS TABLE(id uuid, first_name text, last_name text, member_number text, branch_name text, group_name text, last_seen date, days_since integer) LANGUAGE sql STABLE AS $$
  ...
  WHERE m.org_id = p_org_id AND p_org_id = get_my_org_id() AND m.membership_status = 'active'
...
RETURNS TABLE(month text, total numeric) LANGUAGE sql STABLE AS $$
  ...
  FROM transactions
  WHERE org_id = p_org_id AND p_org_id = get_my_org_id()
...
CREATE OR REPLACE FUNCTION public.get_monthly_giving_by_category(p_org_id uuid, p_branch_id uuid DEFAULT NULL::uuid, p_months integer DEFAULT 12)
RETURNS TABLE(month text, tithe numeric, offering numeric, building numeric, other_amount numeric) LANGUAGE sql STABLE AS $$
...
CREATE OR REPLACE FUNCTION public.get_monthly_member_growth(p_org_id uuid, p_branch_id uuid DEFAULT NULL::uuid, p_months integer DEFAULT 12)
RETURNS TABLE(month text, new_members bigint, cumulative bigint) LANGUAGE sql STABLE AS $$
  WITH monthly AS (
...
CREATE OR REPLACE FUNCTION public.get_top_givers(p_org_id uuid, p_branch_id uuid DEFAULT NULL::uuid, p_start date DEFAULT NULL::date, p_end date DEFAULT NULL::date, p_limit integer DEFAULT 25)
RETURNS TABLE(member_id uuid, first_name text, last_name text, member_number text, branch_name text, total_given numeric, gift_count bigint, last_gift_date date) LANGUAGE sql STABLE AS $$
  SELECT m.id, m.first_name, m.last_name, m.member_number,
    br.name, SUM(t.amount), COUNT(t.id), MAX(t.transaction_date)
  FROM transactions t
  JOIN members m ON m.id = t.member_id
  LEFT JOIN branches br ON br.id = m.branch_id
  WHERE t.org_id = p_org_id AND p_org_id = get_my_org_id() AND t.member_id IS NOT NULL AND t.is_collective IS NOT TRUE
...
RETURNS TABLE(week_start text, present_count bigint, expected_count bigint, rate numeric) LANGUAGE sql STABLE AS $$
  SELECT
